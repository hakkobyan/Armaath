...
export async function sendMessage(roomId:string,senderId:string,content:string,attachment?:PendingAttachment|null){let attachmentPath:string|null=null;try{if(attachment)attachmentPath=await uploadAttachment(roomId,senderId,attachment);const message={chat_room_id:roomId,sender_id:senderId,content:content.trim(),...(attachment?{attachment_path:attachmentPath,attachment_name:attachment.name,attachment_type:attachment.mimeType,attachment_size:attachment.size}:{})};const{data,error}=await supabase.from('messages').insert(message).select().single();if(error)throw error;return data}catch(error){if(attachmentPath)await supabase.storage.from('chat-attachments').remove([attachmentPath]);throw error}}
