-- Repairs chat setup and makes delete permissions explicit.
-- Safe to run after the earlier migrations and safe to run more than once.
...
alter table public.chat_rooms drop constraint if exists chat_rooms_shape_check;
alter table public.chat_rooms add constraint chat_rooms_room_type_check
  check (room_type in ('group', 'global'));
alter table public.chat_rooms add constraint chat_rooms_shape_check
  check (
    (room_type = 'group' and group_id is not null)
    or (room_type = 'global' and group_id is null)
  );

create unique index if not exists one_global_chat_room
  on public.chat_rooms(room_type) where room_type = 'global';
...
alter table public.messages add constraint messages_content_or_attachment_check
  check ((length(trim(content)) between 1 and 2000) or attachment_path is not null);
...
for select to authenticated
using (public.can_access_chat_room(chat_room_id));
...
with check (is_deleted = true and public.can_access_chat_room(chat_room_id));
...
with check (
  ...
  and public.can_access_chat_room(((storage.foldername(name))[1])::uuid)
);
